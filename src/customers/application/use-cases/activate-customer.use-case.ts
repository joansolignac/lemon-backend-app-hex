...
export class ActivateCustomerUseCase {
  private readonly logger = new Logger(ActivateCustomerUseCase.name);
  ...
  async execute(id: string): Promise<void> {
    ...
    this.logger.log(
      `Customer activated: ID=${customer.getId().toPrimitives()}, Name=${customer.getName().toPrimitives()}, NumDocument=${customer.getNumDocument().toPrimitives()}`,
    );
  }
}
